...
export async function generateStaticParams() {
  ...
  return locales.flatMap((locale) =>
    spots.map((s) => ({ locale, slug: s.id }))
  )
}

export default async function DemoSpotPage({
  params,
}: {
  ...
  const spot = await getSpotById(slug)
  if (!spot) notFound()
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-16">
      {/* 掲載イメージバナー */}
      <div className="mb-8 rounded-xl overflow-hidden bg-yellow-400 text-yellow-900 px-6 py-4 flex items-center gap-3 border border-yellow-500">
        <span className="text-xl">📋</span>
        <div>
          <p className="font-bold text-sm tracking-wide">これはデモ表示です</p>
          <p className="text-xs mt-0.5 opacity-80">This is a preview of how your listing will appear on ISHIGAKI NIGHT.</p>
        </div>
      </div>
